...
from .validators import generate_validators_from_mapper, _validate_data
...
class Serializable:
    ...
    def to_dict(self, related=[], ignore=[]):
        ...
        '''
        results = {}
        # Meta param to control value formatting
        formatting = getattr(self.__class__._meta, 'serialize_formatting', {})
        for key, col in self.__mapper__.columns.items():
            name = col.name
            if name not in ignore:
                value = getattr(self, key)
                if name in formatting:
                    results[name] = formatting[name](value)
                elif isinstance(value, (int, str, float, type(None), bool)):
                    results[name] = value
                else:
    ...
    def serialize(self, related=[], ignore=[]):
        ...
        if isinstance(self, Iterable):
            ...
            for i in self:
                assert isinstance(
                    i, Serializable), 'The object is not Serializable.'
                if not related:
                    related = getattr(i.__class__._meta,
                                      'serialize_related', [])
                if not ignore:
                    ignore = getattr(i.__class__._meta, 'serialize_ignore', [])
                results.append(i.to_dict(related=related, ignore=ignore))
            return results
        else:
            assert isinstance(
                self, Serializable), 'The object is not Serializable.'
            if not related:
                related = getattr(self.__class__._meta,
                                  'serialize_related', [])
            if not ignore:
                ignore = getattr(self.__class__._meta, 'serialize_ignore', [])
    ...
    def _parse_args(self, args):
        ...
        for i in args:
            ...
            if temp[0] in parsed:
                if temp[1]:
                    parsed.get(temp[0]).append(temp[1])
            else:
                if temp[1]:
...
class ValidationMeta(DefaultMeta):
    ...
    def __init__(cls, name, bases, d):
        ...
        if hasattr(cls, '__mapper__'):
            ...
            validators, required = generate_validators_from_mapper(
                cls.__mapper__)
            ...
            if hasattr(meta, 'extra_validators'):
                ...
                for k, v in meta.extra_validators.items():
                    meta.default_validators[k] = meta.default_validators.get(k, [
                    ]) + v
